...
class trainButtonsUtil extends StatelessWidget {
  trainButtonsUtil({Key? key, required this.filter, SearchArg? this.searchArg}) : super(key: key);
  final String filter ;
  SearchArg? searchArg ;
  ...
  Future<List<trainModel>> gettrainsApi ()async{
    String uri ;
    if((searchArg != null)&&(globals.refresh == 0)){
        ...
        Expanded(
          child: FutureBuilder(
            ...
            builder: (context , snapshot){
              if(!snapshot.hasData){
                return Text('Loading ...');
              }else {if((globals.refresh == 0)&&(trainList.length == 0) && (searchArg != null )) {
                return Center(
                  child: Text(
                    "No Train is found", style: TextStyle(fontSize: 20),
                  ),
                );
              }
              else{
